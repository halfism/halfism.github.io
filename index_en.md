...
<main class="pt-20">
    ...
    <!-- Development Log -->
    <section id="logs" class="py-20 bg-white dark:bg-[#0d1117] transition-colors duration-300">
        <div class="container mx-auto px-4 md:px-6">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">Project Timeline</h2>
                <p class="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">Tracking every step of progress and innovation</p>
                <div class="w-16 h-1 bg-primary mx-auto rounded-full mt-4"></div>
            </div>

            <div class="max-w-4xl mx-auto">
                <div class="relative border-l-2 border-gray-100 dark:border-gray-800 ml-4 md:ml-6 space-y-12">
                    {% for log in site.data.logs %}
                    <div class="relative pl-8">
                        <!-- Timeline Dot -->
                        <div class="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-white dark:bg-[#0d1117] border-2 border-primary z-10"></div>
                        
                        <div class="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-6">
                            <span class="text-sm font-bold text-primary whitespace-nowrap">{{ log.date }}</span>
                            <div class="card-base p-6 bg-gray-50 dark:bg-[#161b22] w-full">
                                <div class="flex items-center gap-3 mb-3">
                                    {% if log.type == 'feat' %}
                                    <span class="px-2 py-0.5 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider">Feature</span>
                                    {% elif log.type == 'code' %}
                                    <span class="px-2 py-0.5 rounded bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-xs font-bold uppercase tracking-wider">Code</span>
                                    {% elif log.type == 'api' %}
                                    <span class="px-2 py-0.5 rounded bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 text-xs font-bold uppercase tracking-wider">API</span>
                                    {% endif %}
                                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                                        {% if page.lang == 'en' %}{{ log.title_en }}{% else %}{{ log.title_zh }}{% endif %}
                                    </h3>
                                </div>
                                <p class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
                                    {% if page.lang == 'en' %}{{ log.desc_en }}{% else %}{{ log.desc_zh }}{% endif %}
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <!-- GitHub Stats -->
    <section class="py-20 bg-gray-50 dark:bg-[#0d1117]/50 border-t border-gray-100 dark:border-gray-800 transition-colors duration-300">
        <div class="container mx-auto px-4 md:px-6">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">GitHub Stats</h2>
                <p class="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">My open source contributions and coding activity</p>
                <div class="w-16 h-1 bg-primary mx-auto rounded-full mt-4"></div>
            </div>
            ...
            <div class="max-w-4xl mx-auto">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Stats Card -->
                    <div class="gh-stats-card animate-on-scroll">
                        <div class="flex items-center gap-3 mb-6">
                            <i class="fa fa-github text-2xl text-white"></i>
                            <span class="text-white font-bold text-lg">{{ site.github_username }}</span>
                        </div>
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="stat-item">
                                <i class="fa fa-code-commit text-green-400"></i>
                                <div>
                                    <div class="text-2xl font-bold text-white">2k+</div>
                    ...
                    <div class="card-base p-6 bg-white dark:bg-[#161b22] animate-on-scroll animate-delay-100">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="font-bold text-gray-900 dark:text-white">Contribution Activity</h3>
                            <span class="text-sm text-gray-500">Past Year</span>
                        </div>
                        <div class="grid grid-cols-12 gap-1">
                            {% for i in (1..48) %}
                            <div class="h-3 rounded-sm bg-green-{% cycle '100', '200', '300', '400', '500' %} dark:bg-green-{% cycle '900/20', '800/30', '700/40', '600/50', '500/60' %}"></div>
                            {% endfor %}
                        </div>
                        <div class="mt-4 flex items-center gap-4 text-sm text-gray-500">
                            <span class="flex items-center gap-1"><span class="w-3 h-3 rounded bg-green-200 dark:bg-green-800/30"></span> Less</span>
                            <span class="flex items-center gap-1"><span class="w-3 h-3 rounded bg-green-500 dark:bg-green-500/60"></span> More</span>
                        </div>
                    </div>
                </div>

                <!-- Top Languages -->
                <div class="card-base p-6 bg-white dark:bg-[#161b22] mt-6 animate-on-scroll animate-delay-200">
                    <h3 class="font-bold text-gray-900 dark:text-white mb-4">Top Languages</h3>
                    <div class="space-y-3">
                        <div>
                            <div class="flex justify-between mb-1">
                                <span class="text-sm text-gray-700 dark:text-gray-300">JavaScript / TypeScript</span>
                        ...
                        <div>
                            <div class="flex justify-between mb-1">
                                <span class="text-sm text-gray-700 dark:text-gray-300">Go</span>
    ...
    <!-- Certificates -->
    <section id="certificates" class="py-20 bg-white dark:bg-[#0d1117] transition-colors duration-300">
        <div class="container mx-auto px-4 md:px-6">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">Professional Certifications</h2>
                <p class="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">Continuous learning, professional growth</p>
                <div class="w-16 h-1 bg-primary mx-auto rounded-full mt-4"></div>
            </div>

            <div class="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
                {% for cert in site.data.certificates %}
                <a href="{{ cert.url }}" target="_blank" rel="noopener noreferrer" class="certificate-card card-base p-6 bg-white dark:bg-[#161b22] hover:shadow-xl">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="w-12 h-12 rounded-lg {{ cert.color }} flex items-center justify-center">
                            <i class="fa {{ cert.icon }} text-xl"></i>
                        </div>
                        <div>
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ cert.date }}</span>
                            <div class="text-sm text-primary">{{ cert.issuer }}</div>
                        </div>
                    </div>
                    <h3 class="font-bold text-gray-900 dark:text-white mb-2">{{ cert.title_en }}</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">View Certificate →</p>
                </a>
                {% endfor %}
            </div>
        </div>
    ...
    <section class="py-16 bg-gradient-to-r from-blue-600 to-cyan-500 text-white">
        <div class="container mx-auto px-4 md:px-6 text-center">
            <h2 class="text-3xl font-bold mb-4">Want to Know More?</h2>
            <p class="text-lg text-white/90 mb-8 max-w-2xl mx-auto">
                Download my CV for detailed experience and skills information
            </p>
            <div class="flex justify-center gap-4 flex-wrap">
                <a href="/assets/resume_en.pdf" download class="px-8 py-3 rounded-lg bg-white text-blue-600 font-medium hover:bg-gray-100 transition-all shadow-lg">
                    <i class="fa fa-download mr-2"></i> Download CV (English)
                </a>
                <a href="/assets/resume.pdf" download class="px-8 py-3 rounded-lg border-2 border-white text-white font-medium hover:bg-white/10 transition-all">
                    <i class="fa fa-download mr-2"></i> 下载简历 (中文)
                </a>
            </div>
        </div>
    </section>
