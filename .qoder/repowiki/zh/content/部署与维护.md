...
- 修复 _config.yml 中字体配置的 YAML 语法问题
- 优化 en/index.html 模板，移除重复的头部和底部包含
- 更新依赖关系分析以反映新的构建流程
...
**更新** 本版本反映了最新的依赖管理重构，采用更稳定的 github-pages gem 方案，并解决了 Ruby 3.0+ 兼容性问题。
...
**更新** 依赖管理采用 github-pages gem 方案，提供更稳定的版本兼容性和插件组合管理。
...
**更新** 采用 github-pages gem 管理 Jekyll 版本，简化了依赖声明并确保与 GitHub Pages 环境的一致性。
...
**更新** 依赖关系图反映了新的 github-pages gem 和 webrick 依赖管理方案。
...
## 结论
halfism.github.io 采用轻量、可维护的 Jekyll + GitHub Pages + PWA 架构，结合自动化工作流实现稳定高效的发布与运行。通过采用 github-pages gem 的依赖管理方案和 webrick 兼容性支持，进一步提升了构建稳定性与环境一致性。遵循本文的部署与维护指南，可确保构建一致性、运行稳定性与用户体验。
