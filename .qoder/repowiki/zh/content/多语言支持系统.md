...
data["_data/locales/en.yml<br/>英文键值映射"] --> header["_includes/header.html<br/>导航与语言切换"]
data --> footer["_includes/footer.html<br/>页脚文本"]
data --> blog_md["blog/index.md<br/>博客列表页"]
data --> blog_html["blog.html<br/>旧版博客页"]
data --> gallery["gallery.html<br/>画廊页"]
...
- 本地化数据文件
  - _data/locales/en.yml 提供英文键值映射，键空间覆盖导航、英雄区、关于、项目、技能、日志、证书、联系、博客、画廊、搜索、PWA、页脚、文章与通用等模块。
- 模板中的语言选择与渲染
...
participant D as "_data/locales/en.yml"
...
- 文件组织
  - 当前仅包含英文数据文件 en.yml，键空间覆盖导航、英雄区、关于、项目、技能、日志、证书、联系、博客、画廊、搜索、PWA、页脚、文章与通用等模块。
- 字段映射
...
### 新增语言扩展指南
- 创建数据文件
  - 在 _data/locales/ 下新增对应语言代码的 YAML 文件，保持与 en.yml 一致的键空间。
- 更新配置
...
data["_data/locales/*.yml"] --> jekyll
layout["_layouts/default.html"] --> build
includes["_includes/*.html"] --> build
...
- 语言文本未生效
  ...
  - 确认 _data/locales/ 下对应语言文件存在且键空间与 en.yml 一致。
- 语言切换跳转错误
...
## 结论
halfism.github.io 的多语言支持以 Jekyll 配置与数据驱动为核心，结合模板层的条件渲染与导航逻辑，实现了中英双语的稳定运行。通过规范的键空间管理、URL 结构设计与 SEO 优化，系统在可维护性与可扩展性方面表现良好。建议在后续扩展新语言时，遵循本文提供的流程与最佳实践，确保一致的用户体验与无障碍支持。

## 附录
- 实际配置示例路径
  - 站点配置与语言设置：[_config.yml:62-76](file://_config.yml#L62-L76)
  - 英文本地化数据：[_data/locales/en.yml:1-166](file://_data/locales/en.yml#L1-166)
